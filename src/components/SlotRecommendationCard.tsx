...
function timeUntil(iso: string): string {
  ...
  if (diffMs <= 0) return "endet gleich";
  const hours = Math.floor(diffMs / 3_600_000);
  const minutes = Math.floor((diffMs % 3_600_000) / 60_000);
  if (hours >= 24) return `noch ${Math.floor(hours / 24)}d ${hours % 24}h`;
        ...
        {top ? (
          <div className="flex items-center gap-3 rounded-xl border border-primary/30 bg-primary/10 p-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-xs text-muted">{top.reasons.join(" · ")}</p>
            </div>
          </div>
        ) : (
          <p className="text-sm text-muted">Keine Empfehlung verfügbar.</p>
        )}
